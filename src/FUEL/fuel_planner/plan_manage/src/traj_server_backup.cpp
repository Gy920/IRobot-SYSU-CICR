...
//
    // pos_ctl
    // pos_ctl.pose.pose.position.x = traj_cmd.pos(0);
    // pos_ctl.pose.pose.position.y = traj_cmd.pos(1);
    // pos_ctl.pose.pose.position.z = traj_cmd.pos(2);

    // pos_ctl.pose.pose.orientation.w = traj_cmd.yaw;
    // pos_ctl.pose.pose.orientation.x = 0.0;
    // pos_ctl.pose.pose.orientation.y = 0.0;
    // pos_ctl.pose.pose.orientation.z = 0.0;

    // pos_ctl.twist.twist.linear.x = traj_cmd.vel(0);
    // pos_ctl.twist.twist.linear.y = traj_cmd.vel(1);
    // pos_ctl.twist.twist.linear.z = traj_cmd.vel(2);

    // pos_ctl.twist.twist.angular.x = 0;
    // pos_ctl.twist.twist.angular.y = traj_cmd.yawdot;
    // pos_ctl.twist.twist.angular.z = 0;
...
int main(int argc, char** argv) {
  ...
  // pos_ctl_pub  = node.advertise<nav_msgs::Odometry>("/position_control", 50);
