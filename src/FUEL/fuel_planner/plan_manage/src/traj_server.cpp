...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub ,pos_ctl_pub;
nav_msgs::Odometry pos_ctl;
...
//
  ...
  pos_ctl.pose.pose.position.x = odom.pose.pose.position.x;
  pos_ctl.pose.pose.position.y = odom.pose.pose.position.y;
  pos_ctl.pose.pose.position.z = odom.pose.pose.position.z;

  pos_ctl.pose.pose.orientation.w = odom.pose.pose.orientation.w;
  pos_ctl.pose.pose.orientation.x = odom.pose.pose.orientation.x;
  pos_ctl.pose.pose.orientation.y = odom.pose.pose.orientation.y ;
  pos_ctl.pose.pose.orientation.z = odom.pose.pose.orientation.z;

  pos_ctl.twist.twist.linear.x = pos(0);
  pos_ctl.twist.twist.linear.y = pos(1);
  pos_ctl.twist.twist.linear.z = pos(2);

  pos_ctl.twist.twist.angular.x = vel(0);
  pos_ctl.twist.twist.angular.y = vel(1);
  pos_ctl.twist.twist.angular.z = vel(2);

  pos_ctl_pub.publish(pos_ctl);
...
int main(int argc, char** argv) {
  ...
  pos_ctl_pub = node.advertise<nav_msgs::Odometry>("/pos_control", 50);
